...
class DecoderTests extends AnyFreeSpec {
  ...
  "LocalDate Decoder" - {
    ...
    "Successful decoding to Left but failure to parse date" in {
      ...
      val expected = Left(
        DecoderErr(
          s"Failure to parse Local date time: 2012-10-43-5, with exception Text '2012-10-43-5' could not be parsed, unparsed text found at index 10"
        )
      )
      val result = Decoder.DecoderOps(date).as[LocalDate]
      assert(result == expected)
  ...
  "Tuple 3 Decoder" - {
    "Successful decoding" in {
      val jsonObj =
        Json.Object(Map("1" -> Json.String("value 1"), "2" -> Json.String("value 2"), "3" -> Json.Number(123)))
  ...
  "User Decoder" - {
    "Successful decoding" in {
      val jsonObj =
        Json.Object(Map("name" -> Json.String("Mon"), "age" -> Json.Number(24), "kind" -> Json.String("Normal")))
