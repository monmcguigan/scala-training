...
object Parser {
  ...
  def pred(cond: Char => Boolean): Parser[Char] = Parser.from { input =>
  ...
  def string(expected: String): Parser[String] =
    expected.map(Parser.char).foldLeft(Parser.pure("")) { (headParser, tailParser) =>
      headParser.map2(tailParser) { (head, tail) =>
        head + tail
      }
    }
...
sealed trait Parser[+A] {
  ...
  def map2[B, C](other: => Parser[B])(f: (A, B) => C): Parser[C] = Parser.from { input =>
    parse(input) match {
      case Success(a, restA) =>
        other.parse(restA) match {
  ...
  def repSep(sep: String): Parser[List[A]] = (this ~ (string(sep) *> this).rep0) map { case (a, as) =>
    a :: as
  }
